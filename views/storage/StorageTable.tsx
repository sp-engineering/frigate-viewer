...
import {formatSize, useTableStyles} from '../../helpers/table';
  ...
  const data = useMemo(
    () =>
      storage !== undefined
        ? [
            [
              formatSize(storage.recordings.used),
              formatSize(storage.recordings.total),
            ],
  ...
  return (
    <Table borderStyle={{borderWidth: 1}}>
      <TableWrapper style={tableStyles.row}>
        <Cell
          data={intl.formatMessage(messages['location.header'])}
        ...
        />
        <Cell
          data={intl.formatMessage(messages['total.header'])}
          style={tableStyles.header}
          textStyle={tableStyles.headerText}
        />
      </TableWrapper>
