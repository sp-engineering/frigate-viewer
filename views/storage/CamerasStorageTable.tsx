...
export const CamerasStorageTable: FC<ICamerasStorageTableProps> = ({camerasStorage}) => {
  const intl = useIntl();

  const dataHeaders = useMemo(() => Object.keys(camerasStorage), [camerasStorage]);
  ...
  return (
    <Table borderStyle={{borderWidth: 1}}>
      <TableWrapper style={tableStyles.row}>
        <Cell data={intl.formatMessage(messages['camera.header'])} style={tableStyles.mainHeader} textStyle={tableStyles.mainHeaderText} />
        <Cell data={intl.formatMessage(messages['used.header'])} style={[tableStyles.header, {flex:2}]} textStyle={tableStyles.headerText} />
        <Cell data={intl.formatMessage(messages['bandwidth.header'])} style={tableStyles.header} textStyle={tableStyles.headerText} />
      </TableWrapper>
      <TableWrapper style={tableStyles.row}>
